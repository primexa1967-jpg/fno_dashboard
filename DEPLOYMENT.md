...
6. **Frontend on Render (Static Site)** — either add the **`primexa-fno-web`** service from `render.yaml` (Blueprint sync) or create it manually:
   - **New** → **Static Site** → same GitHub repo.
   - **Build command:** `npm ci && npm run build -w @option-dashboard/shared && npm run build -w @option-dashboard/web`
   - **Publish directory:** `apps/web/dist`
   - **Environment:** `VITE_API_BASE_URL=https://primexa-fno-api.onrender.com` (or `https://api.primexalearning.in` if the API uses that domain — must match the URL the browser will call).
   - **Redirects / rewrites:** add a **rewrite** so client-side routing works: `/*` → `/index.html` (see `render.yaml` `routes` on `primexa-fno-web`).
   - After deploy, add the static site URL to the **API** env `CORS_ORIGINS`, comma-separated, e.g.  
     `https://primexalearning.in,https://www.primexalearning.in,https://primexa-fno-web.onrender.com`
   - Custom domain **primexalearning.in** → attach to the **Static Site** in Render; point DNS as Render shows.
7. Other frontends: **Cloudflare Pages** / Netlify / Nginx — same build command and `VITE_API_BASE_URL`; always update `CORS_ORIGINS` on the API.
...
Source: `https://github.com/primexa1967-jpg/fno_dashboard.git`